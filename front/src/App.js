...
class App extends React.Component{
  ...
  async getAllMarketData(){
    ...
    subset.forEach((sub)=>{
      rank.forEach((rare)=>{
        const result = this.expdict[sub] ?? 123;
        if (result===123){
          console.log("key error:",sub);
        }
        else if (Number(this.expdict[sub][rare])!==0){
